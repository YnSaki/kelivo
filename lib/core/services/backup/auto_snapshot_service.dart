...
class AutoSnapshotService {
  ...
  Future<AutoSnapshotResult> createSnapshot() async {
    ...
    try {
      ...
      // Monotonic allocation: the next suffix is derived from the LARGEST
      // name already allocated for this same second, never from probing for
      // free paths. A free unsuffixed path (after an eviction removed it)
      // must not be reused — the unsuffixed name sorts behind every
      // suffixed name of the second, so the freshly committed snapshot would
      // be the eviction walk's "oldest" target a moment later.
      var nextCounter = 1;
      for (final ent in dir.listSync()) {
        if (ent is! File) continue;
        final name = ent.path.split(Platform.pathSeparator).last;
        if (name == '$baseName.zip' && nextCounter == 1) nextCounter = 2;
        if (name.startsWith('$baseName.zip') ||
            !name.startsWith('${baseName}_') ||
            !name.endsWith('.zip')) {
          continue;
        }
        final suffix = int.tryParse(
          name.substring('${baseName}_'.length, name.length - 4),
        );
        if (suffix != null && suffix + 1 > nextCounter) {
          nextCounter = suffix + 1;
        }
      }
      final fileName = nextCounter == 1
          ? '$baseName.zip'
          : '${baseName}_$nextCounter.zip';
