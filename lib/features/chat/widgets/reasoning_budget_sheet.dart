...
/// Off / Auto / Low / Medium / High (/ XHigh / Max) / Custom — the standard
/// reasoning budget choices were mapped to a stepped slider in upstream
/// Kelivo's mobile redesign. Desktop still uses its popover list.
...
class _ReasoningBudgetSheetState extends State<_ReasoningBudgetSheet>
  ...
  void _commitIndex(int index) {
    ...
    unawaited(context.read<SettingsProvider>().setThinkingBudget(value));
  }
  ...
  /// Drag end / cancel: commit and snap from the freshest [position] held by
  /// this State, never from the last built widget. Pointer events can be
  /// processed right after the final drag update but before a rebuild, so
  /// reading the child's rebuilt [position] there could revert an already
  /// committed crossing.
  void _onDragEnd() {
    setState(() => _dragging = false);
    final index = _position.round().clamp(0, _stops.length - 1).toInt();
    _commitIndex(index);
    _animateTo(index);
  }
  ...
  Future<void> _openCustomBudget() async {
    ...
    // Cancel a running snap so its ticks cannot overwrite the custom thumb.
    _snap.stop();
    setState(() {
      _selected = chosen;
          ...
          children: [
            ...
            Padding(
              ...
              child: _EffortSlider(
                ...
                stopSemanticsValues: [for (final s in _stops) s.title],
                dragging: _dragging,
...
class _EffortSlider extends StatelessWidget {
  const _EffortSlider({
    ...
    required this.stopSemanticsValues,
  ...
  /// Title of every stop, used to announce the result of increase/decrease.
  final List<String> stopSemanticsValues;
  final bool dragging;
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    // Screen readers can step the slider even though the widget is hand-drawn.
    final index = position.round().clamp(0, stopCount - 1).toInt();
    return Semantics(
      slider: true,
      value: semanticsValue,
      increasedValue: index < stopCount - 1
          ? stopSemanticsValues[index + 1]
          : null,
      decreasedValue: index > 0 ? stopSemanticsValues[index - 1] : null,
      onIncrease: index < stopCount - 1 ? () => onSnap(index + 1) : null,
      onDecrease: index > 0 ? () => onSnap(index - 1) : null,
      child: LayoutBuilder(
            ...
            child: SizedBox(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  // Invisible per-stop anchored keyed boxes used by tests to
                  // target an individual stop on the track.
                  Positioned.fill(
                    child: Row(
                      children: [
