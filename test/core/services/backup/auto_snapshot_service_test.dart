...
void main() {
  group('AutoSnapshotService', () {
    ...
    test('same-second allocation is monotonic: evicted unsuffixed names are '
        'never reused and the newest snapshots survive', () async {
      final fixed = DateTime(2026, 9, 5, 23, 37, 0);
      final service = _service(root, exporter, clock: () => fixed);
      ...
      for (var i = 0; i < 5; i++) {
        ...
        expect(result.status, AutoSnapshotStatus.created);
        names.add(result.metadata!.fileName);
        // Every reported creation must still exist AFTER its own eviction
        // pass — a reused unsuffixed name (same second, sorted last)
        // would have been deleted here.
        ...
        expect(
          zip.existsSync(),
          isTrue,
          reason: 'snapshot #$i (${result.metadata!.fileName}) survives',
        );
      }

      // 5 distinct snapshots in one second, cap 3: the first two pairs
      // were evicted, the final three are exactly the newest payloads
      // (indices 2..4). The unsuffixed slot was vacated at step 4 but is
      // NOT reused at step 5 — allocation stays monotonic.
      expect(names, [
        'auto_snapshot_20260905T233700.zip',
        'auto_snapshot_20260905T233700_2.zip',
        'auto_snapshot_20260905T233700_3.zip',
        'auto_snapshot_20260905T233700_4.zip',
        'auto_snapshot_20260905T233700_5.zip',
      ]);
      final after = await service.listSnapshots();
      expect(after, hasLength(3));
      expect(after.map((s) => s.fileName), [
        'auto_snapshot_20260905T233700_5.zip',
        'auto_snapshot_20260905T233700_4.zip',
        'auto_snapshot_20260905T233700_3.zip',
      ]);
